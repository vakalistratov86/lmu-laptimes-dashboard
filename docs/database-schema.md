...
База данных содержит **18 таблиц**, объединённых в логические группы:
...
| Аккаунты | `users`, `user_sessions` | Регистрация/вход по email+паролю (`server/auth.ts`) — учётные записи пользователей UI, отдельные от `drivers` (игровых пилотов из логов) |

> **О пометках «FK →» в таблицах ниже:** `shared/schema.ts` не объявляет колонки через Drizzle `.references(...)` — внешние ключи не закреплены ограничениями на уровне БД, это связи по соглашению об именовании (`*_id`), которые приложение поддерживает само. «FK →» здесь описывает именно такую логическую связь, не физическое ограничение Postgres.
...
| `sector1_ms` | INTEGER | nullable | Время сектора 1, мс — может отсутствовать для незасчитанного круга |
| `sector2_ms` | INTEGER | nullable | Время сектора 2, мс |
| `sector3_ms` | INTEGER | nullable | Время сектора 3, мс |
...
| `date_time_unix` | BIGINT | Unix timestamp из XML (`DateTime`) |
...
| `uploaded_by_user_id` | INTEGER | nullable, FK → `users.id` | Кто загрузил файл лога этой сессии (`POST /api/import`) — `NULL` для анонимной загрузки (вход не обязателен для импорта) и для сессий, импортированных до появления этого поля. Используется страницей `/admin` для статистики загрузок по пользователю |
...
Результат каждого реального пилота в рамках конкретной сессии. Для сольной машины (один человек всю сессию) — одна строка на машину, как и раньше. Для **командной гонки со сменой пилота** (эндуранс-заезды, тег `<Swap>` в логе, см. `server/logParser.ts`/`server/importWorker.ts`) — по отдельной строке на КАЖДОГО реального пилота одной машины: поля машины (`team`/`car_number`/`veh_name`/`category`/`position`/`class_position`/`finish_status`) одинаковы у всех строк одной машины, а `laps`/`pitstops`/`best_lap_ms` пересчитаны по факту из СОБСТВЕННЫХ кругов этого пилота (не доверяем `<Laps>`/`<Pitstops>`/`<BestLapTime>` из лога — это агрегаты по машине целиком, включая круги со-пилота).
...
| `laps` | INTEGER NOT NULL | Количество кругов ЭТОГО пилота (не машины целиком при смене пилота) |
| `pitstops` | INTEGER NOT NULL | Количество пит-стопов ЭТОГО пилота |
| `best_lap_ms` | INTEGER | Лучший круг ЭТОГО пилота в миллисекундах |
...
| `stint_start_lap` | INTEGER | Первый круг стинта этого пилота за рулём машины. `NULL` — машину вёл один человек всю сессию |
| `stint_end_lap` | INTEGER | Последний круг стинта. При появлении одного и того же пилота в 2+ несмежных стинтах (свап туда-обратно) — `min(startLap)`/`max(endLap)` по всем его стинтам (лоссово сжатый диапазон, не список интервалов) |
| `stint_start_sec` | REAL | Секунда от старта сессии, когда этот пилот сел за руль (из `<DriverChange>`). `NULL`, если стинт с начала сессии или пара `<DriverChange>` не найдена |
| `stint_end_sec` | REAL | Секунда от старта, когда пилота сменили. `NULL`, если стинт до конца сессии |
...
Регистрация/вход по email + паролю (`server/auth.ts`). Учётная запись пользователя UI — не игровой пилот из логов (`drivers`); связь между ними не реализована.
...
| --------- | ----- | ------------- | ---------- |
| `id` | SERIAL | PK | Первичный ключ |
| `email` | TEXT | NOT NULL, UNIQUE | Email, хранится в нижнем регистре (нормализуется на входе Zod-схемой) |
| `password_hash` | TEXT | NOT NULL | scrypt-хэш пароля в формате `"salt:hash"` (`node:crypto`, без внешней зависимости) — пароль в открытом виде не хранится |
| `display_name` | TEXT | NOT NULL | Отображаемое имя |
| `created_at` | BIGINT | NOT NULL | Unix ms |
| `is_admin` | INTEGER | NOT NULL, default 0 | `1` — учётная запись администратора (доступ к странице `/admin`: список пользователей, статистика загрузок, размер БД, удаление сессий). Выставляется только через `POST /api/admin/promote` (требует `ADMIN_TOKEN`) — обычная регистрация не может назначить себе эту роль |

### `user_sessions` — Сессии входа

Случайный токен в httpOnly-cookie (`lmu_session`), не JWT — строка удаляется из таблицы при logout, что отзывает сессию мгновенно, без чёрного списка токенов.

| Колонка | Тип | Ограничение | Описание |
| --------- | ----- | ------------- | ---------- |
| `id` | TEXT | PK | Случайный токен сессии (не PK-инкремент) |
| `user_id` | INTEGER | NOT NULL, индекс | FK → `users.id` |
| `created_at` | BIGINT | NOT NULL | Unix ms |
| `expires_at` | BIGINT | NOT NULL | Unix ms — TTL 30 дней от создания |
...
lap_times ←── drivers                                            │
  ↑ (session_id, nullable)                                       │
sessions ←──────────────────────────────────────────────────────┘
  ↑ (session_id, nullable)          │ 1                 (track_id)
  │                                 │ N
import_jobs                        ├─→ session_results
                                    │       │ 1
                                    │       │ N
...
telemetry_import_jobs
  ↑ (telemetry_session_id, nullable)
telemetry_sessions
  │ 1
  │ N
  └─→ telemetry_channels
...
> **Аккаунты пользователей:** `users`/`user_sessions` не связаны ни с одной из таблиц выше — учётная запись пользователя UI не сопоставлена с `drivers` (игровым пилотом из логов).
>
...
>
> **`import_jobs`/`telemetry_import_jobs` → `sessions`/`telemetry_sessions`:** связь необязательная — `session_id`/`telemetry_session_id` проставляется в задании импорта уже после того, как основная запись создана внутри той же транзакции, и остаётся `NULL` при `status = "failed"`.
