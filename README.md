...
- **Telemetry** — список импортированных записей телеметрии (`.duckdb`-файлы LMU) с датой/временем заезда, трассой и пилотом; **Telemetry Detail** — карта трассы (GPS-трек круга, для откалиброванных трасс — поверх реального спутникового снимка с зумом/паном; траектория раскрашена по скорости, подписаны макс./мин. скорость у каждого поворота) + графики каналов телеметрии (скорость, педали и др.) с выбором круга и синхронизированным зумом/легендой.
...
│       │                #   TelemetryChart, TelemetryLapPicker), UI (shadcn)
│       ├── hooks/       # Кастомные React-хуки
│       └── lib/         # API-хуки, форматирование времён (format.ts), classStyles.ts
│                        #   (единый источник цветов/подписей типов сессии, классов
│                        #   машин и медалей), sessionDetailSelectors.ts, telemetryGeo.ts,
│                        #   telemetrySpeed.ts (профиль скорости по кругу),
│                        #   trackMapCalibration.ts (GPS -> пиксели спутникового снимка)
...
>
> По ссылке «Демо» — работающий инстанс приложения с реальными импортированными данными автора, а не сгенерированные иллюстративные значения: приложение не подставляет фейковые/демонстрационные данные ни в одном разделе (см. «Модель данных» выше).
