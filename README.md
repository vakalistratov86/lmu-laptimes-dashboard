...
├── migrations/          # SQL-миграции БД (сгенерированы drizzle-kit из shared/schema.ts),
│                        #   применяются автоматически при старте сервера (server/migrate.ts)
...
├── Dockerfile           # Образ приложения, применяет миграции при старте
...
Схема (`shared/schema.ts`) применяется к PostgreSQL через версионированные SQL-миграции (`migrations/*.sql`, сгенерированные `drizzle-kit`), а не вручную:

1. После изменения `shared/schema.ts` сгенерируйте файл миграции: `npm run db:generate` — создаст `migrations/NNNN_<name>.sql` и обновит `migrations/meta/`. Закоммитьте оба вместе с изменением схемы.
2. Миграции применяются автоматически при каждом старте сервера (`server/migrate.ts`, вызывается до `registerRoutes()`) через `drizzle-orm`'s migrator — уже применённые миграции отслеживаются в служебной таблице `drizzle.__drizzle_migrations` и повторно не выполняются, так что рестарт сервера всегда безопасен.
3. Отдельно накатывать схему руками (`npm run db:push`) не требуется — это запасной инструмент для быстрых экспериментов в dev, который меняет БД в обход истории миграций, не для постоянного использования.

**Существующие БД, развёрнутые до перехода на этот механизм** (таблицы уже созданы старым ручным `CREATE TABLE IF NOT EXISTS`): при первом запуске обновлённого кода `server/migrate.ts` сам обнаруживает такую БД (таблицы приложения есть, а `drizzle.__drizzle_migrations` ещё нет) и помечает начальную миграцию уже применённой, не выполняя её `CREATE TABLE` повторно — никаких ручных действий не требуется, данные не затрагиваются.
...
Схема БД применяется автоматически при старте (см. «Миграции БД» ниже) — отдельно накатывать её не нужно. Приложение будет доступно на `http://localhost:5000`.
...
| `npm run db:generate` | Сгенерировать SQL-файл миграции из изменений в `shared/schema.ts` |
| `npm run db:push` | Ручной прямой push схемы в БД, в обход миграций (только для быстрых экспериментов в dev, см. «Миграции БД») |
