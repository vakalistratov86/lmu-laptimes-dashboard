...
# 3. Запустите в режиме разработки — таблицы БД применяются автоматически
npm run dev
```
...
├── migrations/          # SQL-миграции БД (drizzle-kit generate), см. «Миграции БД»
...
Если PR меняет `shared/schema.ts` (новая таблица/колонка, смена типа и т.п.):

```bash
# Сгенерировать SQL-файл миграции из изменений в схеме
npm run db:generate
```

- Закоммитьте сгенерированный `migrations/NNNN_<name>.sql` вместе с изменением `shared/schema.ts` — миграция применится автоматически на следующем старте сервера (`server/migrate.ts`), отдельного шага при деплое не требуется
- Не используйте `npm run db:push` для постоянных изменений — он меняет БД напрямую, в обход истории миграций, и рассинхронизирует `migrations/` с реальным состоянием БД. Годится только для одноразовых экспериментов в собственной dev-БД
- Не редактируйте уже закоммиченные файлы в `migrations/` — если нужно исправить ошибку в уже смерженной миграции, добавьте новую миграцию поверх
