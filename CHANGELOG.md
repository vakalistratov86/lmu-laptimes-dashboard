...
- **Схема БД теперь применяется через версионированные drizzle-kit-миграции вместо ручного SQL.** Раньше структура таблиц дублировалась в двух не синхронизируемых автоматически местах — `shared/schema.ts` (Drizzle) и написанный руками raw SQL в `server/migrate.ts` (`CREATE TABLE IF NOT EXISTS` + точечные `ALTER TABLE`/`DO $$...$$` под конкретные исторические правки). Теперь:
  - `npm run db:generate` генерирует версионированный SQL-файл миграции из diff'а `shared/schema.ts` (`migrations/NNNN_<name>.sql`, коммитится в git — настоящая, читаемая в PR история изменений схемы вместо только диффа кода)
  - `server/migrate.ts` больше не содержит ручного SQL — вызывает `drizzle-orm`'s `migrate()`, который применяет только неприменённые миграции, отслеживая это в служебной таблице `drizzle.__drizzle_migrations`; безопасно вызывать на каждом старте сервера, как и раньше
  - **Уже развёрнутые БД** (таблицы созданы старым ручным путём, `drizzle.__drizzle_migrations` ещё нет) переводятся на новую систему автоматически и без потери данных: `baselineExistingDatabase()` в `server/migrate.ts` обнаруживает такую БД по наличию таблиц приложения и отсутствию таблицы учёта миграций, помечает начальную сгенерированную миграцию уже применённой (не выполняя повторно её `CREATE TABLE`) — никакого ручного вмешательства не требуется. Проверено локально: свежая БД (создание с нуля), уже существующая БД с реальными данными (бейзлайн без потерь) и повторный запуск (идемпотентность) — все три сценария отработали корректно
  - `npm run db:push` оставлен как есть, но перестал быть рекомендуемым способом — теперь это инструмент для одноразовых экспериментов в dev, в обход истории миграций
  - `Dockerfile` теперь копирует `migrations/` в продакшен-образ (без этого `server/migrate.ts` не нашёл бы файлы миграций в контейнере) и обновлён комментарий про `shared/`/`drizzle.config.ts` (нужны только для ручной диагностики `drizzle-kit` внутри контейнера, не для регулярного деплоя)
