...
- `TrackMap.tsx`: контур Bahrain Outer Circuit давал неверное положение старт/финиша (маркер стоял в произвольной точке хордовой связки, не совпадающей с реальной линией старта) — контур переоцифрован из тех же GPS-точек, что и Bahrain Grand Prix Circuit, с общим для обеих конфигураций стартом на пит-стрейте; техническая связка поворотов 4-13 заменена одним прямым перегоном (реальная объездная дорога), итоговая длина 3510.7 м совпадает с паспортной (3511 м)
