...
const TRACK_HISTORY: Record<Locale, Record<string, TrackHistoryEntry>> = {
  ru: {
    ...
    Bahrain: {
      builtYear: 2004,
      renovations: [{ year: 2010, note: "Добавлена внешняя трасса для длинной (Endurance) конфигурации" }],
    },
    Portimão: {
      builtYear: 2008,
    ...
    Interlagos: {
      builtYear: 1940,
      renovations: [{ year: 1990, note: "Трасса сокращена с 7.8 км до текущей конфигурации" }],
    },
    COTA: {
      builtYear: 2012,
      renovations: [{ year: 2019, note: "Полная переукладка асфальтового покрытия" }],
    },
    Silverstone: {
      builtYear: 1948,
    ...
    Lusail: {
      ...
      renovations: [{ year: 2021, note: "Реконфигурация трассы под требования Формулы 1" }],
    },
    "Fuji Speedway": {
      builtYear: 1966,
      renovations: [{ year: 2005, note: "Масштабная реконструкция по проекту Германа Тильке" }],
    },
    Sebring: {
      builtYear: 1950,
  ...
  en: {
    ...
    Bahrain: {
      builtYear: 2004,
      renovations: [{ year: 2010, note: "Outer circuit added for the longer Endurance layout" }],
    },
    Portimão: {
      builtYear: 2008,
    ...
    Interlagos: {
      builtYear: 1940,
      renovations: [{ year: 1990, note: "Circuit shortened from 7.8 km to its current configuration" }],
    },
    COTA: {
      builtYear: 2012,
      renovations: [{ year: 2019, note: "Full asphalt resurfacing" }],
    },
    Silverstone: {
      builtYear: 1948,
    ...
    Lusail: {
      ...
      renovations: [{ year: 2021, note: "Circuit reconfigured to Formula 1 requirements" }],
    },
    "Fuji Speedway": {
      builtYear: 1966,
      renovations: [{ year: 2005, note: "Extensively rebuilt to a Hermann Tilke design" }],
    },
    Sebring: {
      builtYear: 1950,
  ...
  if (!track) {
    return (
      <div className="py-12 text-center text-muted-foreground">
        {t("trackDetail.notFound")}.{" "}
        <Link href="/tracks" className="text-primary">
          {t("trackDetail.notFoundBack")}
        </Link>
      </div>
    );
  }
      ...
      <Card className="overflow-hidden">
        <div className="border-b border-border bg-secondary/40 px-5 py-4">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
            <span className="flex items-center gap-1">
              <MapPin size={14} /> {translateCountry(track.country, locale)}
            </span>
            <span className="flex items-center gap-1">
              <Ruler size={14} /> {track.lengthKm} {t("trackDetail.km")}
            </span>
            <span className="flex items-center gap-1">
              <RotateCw size={14} /> {track.turns} {t("trackDetail.turns")}
            </span>
          </div>
        </div>
        ...
        {(showInfoColumn || showMap) && (
          <div className={showInfoColumn && showMap ? "grid md:grid-cols-[1.05fr_1fr]" : "grid"}>
            {showInfoColumn && (
              <div
                className={`p-5 ${showMap ? "order-2 border-t border-border md:order-none md:border-t-0 md:border-r" : ""}`}
              >
                {history && (
                  <>
                    ...
                    <ul className="mb-1">
                      <li className="relative border-l-2 border-border py-0 pb-3.5 pl-5 text-sm before:absolute before:-left-[5px] before:top-1 before:h-2 before:w-2 before:rounded-full before:bg-primary before:content-['']">
                        ...
                        <span className="text-muted-foreground">
                          {history.openedNote ?? t("trackDetail.trackOpened")}
                        </span>
                      </li>
                      {history.renovations.map((r, i) => (
                        <li
                          key={r.year}
                          className={`relative border-l-2 py-0 pl-5 text-sm ${i === history.renovations.length - 1 ? "border-transparent pb-0" : "border-border pb-3.5"} before:absolute before:-left-[5px] before:top-1 before:h-2 before:w-2 before:rounded-full before:bg-primary before:content-['']`}
                        >
                ...
                {fact && (
                  <div
                    className={`flex items-start gap-3 rounded-md border border-primary/20 bg-primary/5 px-4 py-3 ${history ? "mt-5" : ""}`}
                  >
                    <Lightbulb size={16} className="mt-0.5 shrink-0 text-primary" />
                    <div>
                      <span className="text-xs font-semibold uppercase tracking-widest text-primary">
                        {t("trackDetail.trivia")}
                      </span>
        ...
        <>
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <Stat
              ...
              value={formatLap(stats.best.lapMs)}
              sub={stats.best.driverName}
            />
