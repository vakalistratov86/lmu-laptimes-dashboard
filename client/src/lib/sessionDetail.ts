 ...
 */
export function buildSessionDetailViewModel(params: BuildSessionDetailViewModelParams): SessionDetailViewModel {
  const { session, laps, backHref } = params;

  const hasLapData = Array.isArray(laps) && laps.length > 0;

  const normalizedSessionType = normalizeSessionType(String(session.sessionType ?? session.type ?? ""));
