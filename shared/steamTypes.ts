...
export interface SteamAppCard {
  ...
  /** Только уже фактически вышедший контент — трассы будущих ещё не выпущенных паков сюда не подставляются (см. §3.12) */
  tracks: string[];
   ...
   * уже вышедших отдельных паков (см. `includedDlc`), а не описывают
   * собственный контент.
   */
  isPass: boolean;
  /**
   * Только для `isPass === true`: названия отдельных DLC-паков, УЖЕ
   * вышедших и входящих в подписку на сегодня (не полный анонсированный
   * план, если часть паков ещё не выпущена — см. server/steamCatalog.ts).
   * Пустой массив для не-Pass карточек.
   */
  includedDlc: string[];
  /**
